...
def get_user_preferences(request):
    if request.user.is_authenticated:
        return {
            ...
            "show_timer": request.user.show_timer,
            "shuffle_questions": request.user.shuffle_questions,
            "ai_explanation": request.user.ai_explanation,
            "site_language": normalize_language_code(request.user.site_language),
        }
    return PREFERENCE_DEFAULTS.copy()
...
=======
    ...
    f"models/gemini-2.5-flash:generateContent?key={api_key}"
        ...
        if response.status_code != 200:
            return JsonResponse({
                'ok': False,
                'error': result.get('error', {}).get('message', 'Gemini API қатесі шықты.')
            }, status=500)
...
<<<<<<< HEAD
    ...
    return render(request, "accounts/login.html", build_page_context(request, {"form": form}))
=======
    return render(request, 'accounts/login.html', build_page_context(request, {'form': form, 'error': error}))
>>>>>>> 2fb66b60e5cd0fb4e449e2c10377c89bb53b022d
